...
static const bool kShowTiming = true;      // set to "true" for debugging
...
size_t FrameOutput::readFramePixels(void* ptr, size_t size){
    ...
    GLenum glErr;
    assert(mEglWindow.getWidth()* mEglWindow.getHeight() * kGlBytesPerPixel <= size);
    glReadPixels(0, 0, mEglWindow.getWidth(), mEglWindow.getHeight(), GL_RGBA, GL_UNSIGNED_BYTE, ptr);
    if ((glErr = glGetError()) != GL_NO_ERROR) {
        ALOGE("glReadPixels failed: %#x", glErr);
        return UNKNOWN_ERROR;
    }
    reduceRgbaToRgb((uint8_t *)ptr, mEglWindow.getWidth()* mEglWindow.getHeight());
    if (kShowTiming) {
        pixWhenNsec = systemTime(CLOCK_MONOTONIC);
    }
