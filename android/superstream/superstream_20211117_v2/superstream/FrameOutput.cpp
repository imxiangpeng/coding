...
status_t FrameOutput::createInputSurface(int width, int height,
    ...
    mGlConsumer->setName(String8("superstream virtual display"));
...
status_t FrameOutput::doComposition(long timeoutUsec) {
    Mutex::Autolock _l(mMutex);
    //ALOGV("doComposition %ld\n", timeoutUsec);
...
size_t FrameOutput::readFramePixels(void* ptr, size_t size){
      // GLES only guarantees that glReadPixels() will work with GL_RGBA, so we
    // need to get 4 bytes/pixel and reduce it.  Depending on the size of the
    // screen and the device capabilities, this can take a while.
    int64_t startWhenNsec, pixWhenNsec, endWhenNsec;
    ...
    assert(mEglWindow.getWidth()* mEglWindow.getHeight() * (kGlBytesPerPixel + kOutBytesPerPixel) <= size);


    size_t shiftLen = mEglWindow.getWidth()* mEglWindow.getHeight() * kOutBytesPerPixel;

    uint8_t* rgbaPtr = (uint8_t*) ptr + shiftLen;

    glReadPixels(0, 0, mEglWindow.getWidth(), mEglWindow.getHeight(), GL_RGBA, GL_UNSIGNED_BYTE, rgbaPtr);
    if ((glErr = glGetError()) != GL_NO_ERROR) {
        ALOGE("glReadPixels failed: %#x", glErr);
        return UNKNOWN_ERROR;
    }
    ...
    const uint8_t* readPtr = rgbaPtr;
    uint8_t* rgbPtr = (uint8_t*)ptr;
    for (unsigned int i = 0; i < mEglWindow.getWidth()* mEglWindow.getHeight() ; i++) {
        *rgbPtr++ = *readPtr++;
        *rgbPtr++ = *readPtr++;
        *rgbPtr++ = *readPtr++;
        readPtr++;
    ...
    if (kShowTiming) {
        endWhenNsec = systemTime(CLOCK_MONOTONIC);
        //ALOGD("%s(%d) got pixels (get=%.3f ms, reduce=%.3fms)", __FUNCTION__, __LINE__,
        //(pixWhenNsec - startWhenNsec) / 1000000.0,
        //(endWhenNsec - pixWhenNsec) / 1000000.0);
    }

    return mEglWindow.getWidth()* mEglWindow.getHeight() * kOutBytesPerPixel;//kGlBytesPerPixel;
}
