...
void binder_dump_txn(struct binder_transaction_data *txn)
{
    ...
    while (count--) {
        ...
        fprintf(stderr,"  -  flags %08x  ptr %016"PRIx64"  cookie %016"PRIx64"\n",
...
static void binder_free_buffer(struct binder_state *bs,
...
static void binder_send_reply(struct binder_state *bs,
...
uint32_t bio_get_ref(uint32_t type, struct binder_io *bio)
{
    struct flat_binder_object *obj;

    obj = _bio_get_obj(bio);
    if (!obj) {
		printf("can not got obj\n");
        return 0;
	}
	printf("type:BINDER_TYPE_FD:0x%x, BINDER_TYPE_HANDLE:%x0x\n", BINDER_TYPE_FD, BINDER_TYPE_HANDLE);

    printf("%s(%d): type:0x%x, obj hdr type:0x%x, handle:%d\n", __FUNCTION__, __LINE__, type, obj->hdr.type, obj->handle);
    if (type != BINDER_TYPE_FD && type != BINDER_TYPE_HANDLE) {
	  printf("not match type:BINDER_TYPE_FD:0x%x, BINDER_TYPE_HANDLE:%x0x\n", BINDER_TYPE_FD, BINDER_TYPE_HANDLE);
      //return 0;
    }

#if 0
    if (obj->hdr.type == type /*BINDER_TYPE_HANDLE*/)
        return obj->handle;
#endif
    return obj->handle;
    return 0;
