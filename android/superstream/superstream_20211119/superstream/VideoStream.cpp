...
static const int kGlBytesPerPixel = 4;
...
status_t VideoStream::acquireBuffer() {
    ...
    if (ret != 0) {
        ALOGW("VideoStream::%s(%d): warning .....wait lock timeout ret:%d\n", __FUNCTION__, __LINE__, ret);
    }
    return ret;
}
...
bool VideoStream::threadLoop() {
    ...
    int64_t delay = VIDEOSTREAM_CAPTURE_TIME_PER_FRAME_US - ((systemTime(CLOCK_MONOTONIC) - startTime) / 1000);
    
    if (delay > 0) {
        usleep(delay);
    }

    // usleep(VIDEOSTREAM_CAPTURE_TIME_PER_FRAME_US);
