...
#if defined(STUB_INPUT_STREAM_PROVIDER_FILE)
#include <sys/types.h>
#include <sys/stat.h>
#include <fcntl.h>
#endif
                       ...
                       size_t bytes)
{
    ...
    if (in->provider_fd < 0) {
        struct stat sb;
        if (lstat(STUB_INPUT_STREAM_PROVIDER_FILE_PATH, &sb) != 0) {
            // not provider file found, leave mute data, do not reset bytes
            ALOGE("can not open file:%s with error:%s\n", STUB_INPUT_STREAM_PROVIDER_FILE_PATH, strerror(errno));
            goto err;
        }
        // we should open stream
        in->provider_fd = open(STUB_INPUT_STREAM_PROVIDER_FILE_PATH, O_RDONLY);
        if (in->provider_fd < 0) {
            ALOGE("can not open file:%s with error:%s\n", STUB_INPUT_STREAM_PROVIDER_FILE_PATH, strerror(errno));
            bytes = 0;
            goto err;
    ...
    ssize_t len = read(in->provider_fd, buffer, bytes);
    ALOGD("in_read: read data from stream bytes %zu", len);

    if ((size_t)len != bytes) {
        ALOGE("can not read valid data:%zu-%zu\n", len, bytes);
    }
                                  ...
                                  audio_source_t source __unused)
{
    ...
    *stream_in = NULL;
    ...
    ALOGV("adev_open_input_stream: **** sample_rate: %u, channels: %x, format: %d,"
          "frames: %zu", config->sample_rate, config->channel_mask, config->format,
          in->frame_count);
