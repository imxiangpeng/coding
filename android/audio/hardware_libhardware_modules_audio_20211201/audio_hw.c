...
// mxp, 20211201, read data from socket buffer
#define STUB_INPUT_STREAM_PROVIDER_SOCK

// mxp, 20211126, read input stream from file
// #define STUB_INPUT_STREAM_PROVIDER_FILE

#define STUB_INPUT_STREAM_PROVIDER_FILE_PATH "/data/local/tmp/provider.wav"

#if defined(STUB_INPUT_STREAM_PROVIDER_SOCK)
...
#if defined(STUB_INPUT_STREAM_PROVIDER_SOCK)
    if (in->provider_sock != NULL) {
        size_t l = 0;
        int max_loop = 1;
        while (l < bytes && max_loop-- > 0) {
            ssize_t result = sock_stream_read(in->provider_sock, (void*)((unsigned int)buffer + l), bytes -l);
            l += result;
            if (l == 0) {
                //usleep(20 * 1000);
            }
        }
        // ALOGD("in_read: read data from stream bytes %zu vs %zu, standby:%d, sleep_time:%lld, frame size:%zu, sample rate:%zu", l, bytes, standby, sleep_time, audio_stream_in_frame_size(stream), in_get_sample_rate(&stream->common));
...
#if defined(STUB_INPUT_STREAM_PROVIDER_SOCK)
    in->provider_sock = sock_stream_init();
    in->sock_started = 0;
    sock_stream_open(in->provider_sock);
#endif

    ALOGD("adev_open_input_stream: sample_rate: %u, channels: %x, format: %d,"
...
#if defined(STUB_INPUT_STREAM_PROVIDER_SOCK)
    struct stub_stream_in *s = (struct stub_stream_in *)in;
    sock_stream_close(s->provider_sock);
    sock_stream_destroy(s->provider_sock);
    s->provider_sock = NULL;
    s->sock_started = 0;
#endif
