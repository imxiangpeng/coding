...
int detect_jerk_condition(DataPoint *window, int size) {
    ...
    return variance > JERK_VARIANCE_THRESHOLD; // 仅基于方差阈值判断抖动
}
...
void update_motion_state(MotionState *current_state, double *start_time, DataPoint *window, int size, int *state_duration_counter, MotionState *last_stable_state) {
    ...
    if (detect_jerk_condition(window, size)) {
        ...
        new_state = STATE_CONSTANT_SPEED;
    }
    ...
    if (*current_state != new_state) {
        if (*state_duration_counter >= STATE_DURATION_THRESHOLD || new_state == STATE_JERK || new_state == STATE_STATIC) { // 静止也立即切换
            ...
            if (new_state != STATE_JERK && new_state != STATE_STATIC) { // 记录非抖动和非静止状态
                *last_stable_state = new_state;
            }
            *current_state = new_state;
            *start_time = window[size - 1].time;
            *state_duration_counter = 0; // 重置状态持续时间计数器
        } else {
            (*state_duration_counter)++; // 状态持续时间计数器递增
        }
    } else {
        *state_duration_counter = 0; // 状态未改变，重置状态持续时间计数器
    }
}


int main(int argc, char** argv) {
    FILE *file = fopen(argv[1]/*"add-lineaccelaration.csv"*/, "r"); // 使用您的文件名, 修改为 "add-lineaccelaration.csv"
    if (!file) {
        perror("文件打开失败");
        return 1;
    }
    ...
    int state_duration_counter = 0; // 状态持续时间计数器
    double ag_buffer[WINDOW_SIZE] = {0.0}; // 移动平均滤波器的缓冲区  - 添加缓冲区定义
    int buffer_index = 0; // 移动平均滤波器的缓冲区索引 - 添加索引定义
    double smoothed_ag = 0.0; // 平滑后的加速度值 - 添加 smoothed_ag 定义
    double prev_smoothed_ag = 0.0; // 上一个平滑后的加速度值 - 添加 prev_smoothed_ag 定义
    double time, dt, accel_x, accel_y, accel_z, union_g, gyro_x, gyro_y, gyro_z, pressure, temp, ag; // 添加缺失的变量声明

    fgets(line, sizeof(line), file); // 跳过表头
    bool first_line = true; // 添加 first_line 标志
    ...
    while (fgets(line, sizeof(line), file)) {
        DataPoint new_data;
        if (sscanf(line, "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf", // 修改 sscanf 以正确解析所有列
                   &time, &dt, &accel_x, &accel_y, &accel_z, &union_g, &gyro_x, &gyro_y, &gyro_z, &pressure, &temp, &ag) != 12) { // 检查解析结果
            fprintf(stderr, "数据解析错误，跳过行: %s", line);
            continue;
        }
        new_data.time = time; // 填充 new_data.time
        new_data.ag = ag;     // 填充 new_data.ag
        ...
        new_data.ag = smoothed_ag; // 使用平滑后的加速度值
        ...
        if (count < JERK_WINDOW_SIZE) {
            ...
            update_motion_state(&current_state, &start_time, window, JERK_WINDOW_SIZE, &state_duration_counter, &last_stable_state);
        }
