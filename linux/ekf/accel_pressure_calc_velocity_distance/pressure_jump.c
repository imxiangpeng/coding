...
static enum motion_state _running_motion_state = STOPPED;
...
void process_csv(const char *filename) {
    ...
    snprintf(buf, sizeof(buf), "accel,accel_filter,velocity,distance,pressure,temp,altitude,high,delat_accel,pressure_high_remove_jump\n");
    ...
    while (fgets(line, MAX_LINE_LENGTH, file)) {
        ...
        if (pressure_prev != 0) {
            if (fabs(pressure - pressure_prev) > 100) {
                pressure_jump_height += fabs(calculate_height_difference(pressure_prev, pressure, temp));
                printf("mxp ................................jump pressure:  %f -> %f, height: %f\n", pressure_prev, pressure, pressure_jump_height);
        ...
        snprintf(buf, sizeof(buf), "%f,%f,%f,%f,%f,%f,%f,%f,%f,%f\n", ag, self.ekf.x[2], self.ekf.x[1], self.ekf.x[0], pressure, temp, altitude, altitude - default_hight, self.ekf.x[3], altitude - default_hight - pressure_jump_height);
