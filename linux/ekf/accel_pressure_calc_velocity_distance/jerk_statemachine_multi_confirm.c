...
#define JERK_THRESHOLD 0.01     // 抖动方差阈值
#define JERK_WINDOW 20           // 抖动判断窗口大小
#define CONFIRMATION_COUNT 2    // 状态变更的确认次数
...
void update_state(MotionState *current_state, double *start_time, DataPoint *window, int size) {
    static MotionState pending_state = STATIC;
    ...
    if (new_state == pending_state) {
        ...
        if (confirmation_counter >= CONFIRMATION_COUNT && *current_state != new_state) {
